...
const hasTestDatabase = Boolean(process.env.TEST_DB_HOST && process.env.TEST_DB_NAME && process.env.TEST_DB_USER);
...
describeWithDatabase('Treenvite API (e2e)', () => {
  ...
  it('creates the synchronized schema and responds to health checks', async () => {
    await request(app.getHttpServer()).get('/api/v1').expect(200).expect(({ body }) => {
      expect(body).toEqual(expect.objectContaining({ status: 'ok', service: 'treenvite-api' }));
    });
  });
  ...
  it('registers an account', async () => {
    await request(app.getHttpServer())
      ...
      .send({ name: 'Prueba Treenvite', email: `test-${Date.now()}@example.com`, password: 'correct-horse-123' })
      .expect(201);
  });
});
