...
export class AuthService {
  ...
  async register(dto: RegisterDto): Promise<{ message: string }> {
    ...
    if (await this.users.exists({ where: { email } })) {
      throw new ApiException(HttpStatus.CONFLICT, 'EMAIL_ALREADY_EXISTS', 'Ya existe una cuenta con este correo');
    }

    const user = await this.users.save(
      this.users.create({ name: dto.name.trim(), email, avatarUrl: null, emailVerifiedAt: null }),
    );
  ...
  async login(dto: LoginDto, meta: { userAgent?: string; ip?: string }): Promise<SessionResult> {
    ...
    if (!identity.user.emailVerifiedAt) {
      throw new ApiException(HttpStatus.FORBIDDEN, 'EMAIL_NOT_VERIFIED', 'Confirma tu correo antes de entrar');
    }
    return this.createSession(identity.user, meta);
  }

  async googleLogin(dto: GoogleAuthDto, meta: { userAgent?: string; ip?: string }): Promise<SessionResult> {
    const clientId = this.config.getOrThrow<string>('GOOGLE_CLIENT_ID');
    const ticket = await this.google.verifyIdToken({ idToken: dto.credential, audience: clientId });
    const payload = ticket.getPayload();
    if (!payload?.sub || !payload.email || !payload.email_verified) {
      throw new ApiException(HttpStatus.UNAUTHORIZED, 'INVALID_GOOGLE_ACCOUNT', 'Google no confirmó esta cuenta');
    }
  ...
  async refresh(refreshToken: string | undefined, meta: { userAgent?: string; ip?: string }): Promise<SessionResult> {
    if (!refreshToken) throw new ApiException(HttpStatus.UNAUTHORIZED, 'SESSION_EXPIRED', 'La sesión expiró');
    const session = await this.sessions
      ...
      .getOne();
    if (!session) throw new ApiException(HttpStatus.UNAUTHORIZED, 'SESSION_EXPIRED', 'La sesión expiró');
  ...
  async logout(refreshToken?: string): Promise<void> {
    ...
    await this.sessions.update({ tokenHash: sha256(refreshToken), revokedAt: IsNull() }, { revokedAt: new Date() });
  }

  async createCsrf(sessionId: string): Promise<string> {
    const token = randomToken(24);
    await this.sessions.update({ id: sessionId, revokedAt: IsNull() }, { csrfHash: sha256(token) });
    return token;
  }

  async verifyEmail(dto: TokenDto): Promise<{ message: string }> {
    const token = await this.consumeToken(dto.token, AuthTokenType.VERIFY_EMAIL);
  ...
  async resendVerification(dto: EmailDto): Promise<{ message: string }> {
    ...
    if (user && !user.emailVerifiedAt) await this.createAndSendToken(user, AuthTokenType.VERIFY_EMAIL);
  ...
  async resetPassword(dto: ResetPasswordDto): Promise<{ message: string }> {
    const token = await this.consumeToken(dto.token, AuthTokenType.RESET_PASSWORD);
    const identity = await this.identities.findOne({ where: { userId: token.userId, provider: AuthProvider.LOCAL } });
    if (!identity) throw new ApiException(HttpStatus.BAD_REQUEST, 'LOCAL_IDENTITY_NOT_FOUND', 'La cuenta no usa contraseña');
    await this.identities.update(identity.id, { passwordHash: await argon2.hash(dto.password) });
  ...
  async me(userId: string): Promise<User> {
    ...
    if (!user) throw new ApiException(HttpStatus.UNAUTHORIZED, 'USER_NOT_FOUND', 'La cuenta ya no existe');
    return user;
  }

  private async createSession(user: User, meta: { userAgent?: string; ip?: string }): Promise<SessionResult> {
  ...
  private async createAndSendToken(user: User, type: AuthTokenType): Promise<void> {
    ...
    await this.authTokens.save(
      this.authTokens.create({
        ...
        userId: user.id,
        usedAt: null,
        expiresAt: new Date(Date.now() + (type === AuthTokenType.VERIFY_EMAIL ? 24 : 1) * 60 * 60 * 1000),
      }),
    );
    if (type === AuthTokenType.VERIFY_EMAIL) await this.mail.sendVerification(user.email, user.name, raw);
    else await this.mail.sendPasswordReset(user.email, user.name, raw);
  }

  private async consumeToken(raw: string, type: AuthTokenType): Promise<AuthToken> {
    const token = await this.authTokens.findOne({
      where: { tokenHash: sha256(raw), type, usedAt: IsNull(), expiresAt: MoreThan(new Date()) },
    });
    if (!token) throw new ApiException(HttpStatus.BAD_REQUEST, 'TOKEN_INVALID', 'El enlace es inválido o expiró');
