...
export class MembersService {
  ...
  async list(actorId: string, eventId: string) {
    ...
    return this.members.find({ where: { eventId }, relations: { user: true }, order: { createdAt: 'ASC' } });
  }

  async remove(actorId: string, eventId: string, memberId: string) {
    await this.access.requireRole(eventId, actorId, EventRole.OWNER);
    const member = await this.members.findOne({ where: { id: memberId, eventId } });
    if (!member) throw new ApiException(HttpStatus.NOT_FOUND, 'MEMBER_NOT_FOUND', 'El colaborador no existe');
