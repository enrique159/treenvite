...
import { CreateAccessCodeDto, RedeemAccessCodeDto } from './dto/access-code.dto';
...
export class AccessCodesService {
  ...
  constructor(
    @InjectRepository(EventAccessCode) private readonly codes: Repository<EventAccessCode>,
  ...
  async list(actorId: string, eventId: string) {
    ...
    const codes = await this.codes.find({ where: { eventId }, order: { createdAt: 'DESC' } });
    return codes.map((code) => ({ ...code, maskedCode: `••••-${code.codeSuffix}`, codeHash: undefined }));
  }

  async create(actorId: string, eventId: string, dto: CreateAccessCodeDto) {
    await this.access.requireRole(eventId, actorId, EventRole.OWNER);
    const expiresAt = new Date(dto.expiresAt);
    if (expiresAt <= new Date()) throw new ApiException(HttpStatus.BAD_REQUEST, 'INVALID_EXPIRATION', 'La caducidad debe ser futura');
    const plainCode = `TV-${randomToken(6).replace(/[^a-zA-Z0-9]/g, '').slice(0, 8).toUpperCase()}`;
  ...
  async revoke(actorId: string, eventId: string, codeId: string) {
    ...
    if (!code) throw new ApiException(HttpStatus.NOT_FOUND, 'ACCESS_CODE_NOT_FOUND', 'El código no existe');
  ...
  async redeem(userId: string, dto: RedeemAccessCodeDto) {
    ...
    const code = await this.codes.findOne({
      where: { codeHash: this.hashCode(normalized), revokedAt: IsNull(), expiresAt: MoreThan(new Date()) },
      relations: { event: true },
    });
    if (!code) throw new ApiException(HttpStatus.BAD_REQUEST, 'ACCESS_CODE_INVALID', 'El código no existe o expiró');
    let grant = await this.grants.findOne({ where: { accessCodeId: code.id, userId } });
    if (grant) {
