...
export class AccessCodesController {
  ...
  @UseGuards(CsrfGuard)
  create(@CurrentUser() user: AuthenticatedUser, @Param('eventId') eventId: string, @Body() dto: CreateAccessCodeDto) {
    return this.codes.create(user.id, eventId, dto);
  }

  @Delete('events/:eventId/access-codes/:codeId')
  @UseGuards(CsrfGuard)
  revoke(@CurrentUser() user: AuthenticatedUser, @Param('eventId') eventId: string, @Param('codeId') codeId: string) {
    return this.codes.revoke(user.id, eventId, codeId);
  }

  @Post('access-codes/redeem')
  @UseGuards(CsrfGuard)
  redeem(@CurrentUser() user: AuthenticatedUser, @Body() dto: RedeemAccessCodeDto) {
